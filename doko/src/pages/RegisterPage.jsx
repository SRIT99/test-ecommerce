...
const RegisterPage = ({ setCurrentPage, setCurrentUser }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/auth/signup", apiData)
      ...
      if (response.status === 200 || response.status === 201) {
        if (response.data && (response.data.id || response.data.email)) {
          ...
          setErrors({ general: "Registration completed but response format unexpected." })
        }
      } else {
